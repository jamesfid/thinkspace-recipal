...
export default base.extend ta.add(
    ...
    ta.has_many    'tbl:review_sets', reads: { filter: true, name: 'review_sets' }
  ),
  ...
  points_different:           ember.computed 'value.options.points.different', -> 
    points_different = @get('points.different')
    return points_different if ember.isPresent(points_different)
    true
  ...
  reset_points_different:  (value) -> util.set_path_value(@, 'value.options.points.different', true)
