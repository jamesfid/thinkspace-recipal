...
export default base_component.extend
  ...
  model_index: ember.computed 'model', 'items', ->
    items = @get('items')
    model = @get('model')
    return 0 unless ember.isPresent(items) and ember.isPresent(model)
    items.indexOf(model) + 1 # Start at 1 
  ...
  actions: 
    set_qualitative_value: (value) -> @get('manager').set_qualitative_value @get('model.id'), @get('model.feedback_type'),  value
