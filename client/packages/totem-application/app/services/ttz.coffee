...
export default ember.Service.extend
  ...
  get_client_zone_iana: -> 
    has_intl = ember.isPresent(Intl.DateTimeFormat().resolved)
    return Intl.DateTimeFormat().resolved.timeZone if has_intl
    has_moment = ember.isPresent(moment) and ember.isPresent(moment.tz)
    return moment.tz.guess() if has_moment
    'America/Chicago' # Fallback
