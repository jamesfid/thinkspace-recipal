...
import question_manager from 'thinkspace-readiness-assurance/question_manager'
import chat_manager     from 'thinkspace-readiness-assurance/chat_manager'
...
export default ember.Mixin.create
  ...
  init_response: (options) ->
    new ember.RSVP.Promise (resolve, reject) =>
      ...
      if ember.isPresent(response)
        @response = response
        return resolve()
      @tvo.helper.ownerable_view_association_records(@assessment, association: ns.to_p('ra:responses')).then (responses) =>
  ...
  init_get_room: (options) ->
    ...
    room = @ra.phase_ownerable_room() if @is_trat and ember.isBlank(room)
    room
