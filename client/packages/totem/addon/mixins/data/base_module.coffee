...
export default ember.Object.extend
  ...
  get_data: (ownerable=null) ->
    new ember.RSVP.Promise (resolve, reject) =>
      @resolve_source_model().then =>
        if @is_record @get_source_model()
          @get_store_record_data(ownerable).then (data) =>
            resolve(data)
        else
          @get_store_value_data(ownerable).then (data) =>
            resolve(data)
  ...
  get_store_record_data: (ownerable=null) ->
    new ember.RSVP.Promise (resolve, reject) =>
      if ownerable then id = @generate_model_id(null, ownerable) else id = @generate_model_id()
  ...
  get_store_value_data: (ownerable=null) ->
    new ember.RSVP.Promise (resolve, reject) =>
      value = @get_ajax_source_property()
      return resolve({}) if ember.isBlank(value)
  ...
  generate_model_id: (record=null, ownerable=null) ->
    model     = record or @get_source_model()
    ownerable = ownerable or @get_ownerable()
