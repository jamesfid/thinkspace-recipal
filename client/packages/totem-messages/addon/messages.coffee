...
totem_messages = ember.Object.extend
  ...
  message_queue: ember.ArrayController.create()
  ...
  debug_on:        ember.computed -> @is_debug()
  ...
  set_suppress_all:   -> @set('suppress_all', true)
  reset_suppress_all: -> @set('suppress_all', false)
  get_suppress_all:   -> @get('suppress_all')

  suppress_all_messages: (type) ->
    return false if type == 'error'
    config.messages.suppress_all == true or @get_suppress_all()

  is_debug: -> logger.is_debug or false
