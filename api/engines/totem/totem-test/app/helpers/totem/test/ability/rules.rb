module Totem::Test; module Ability; module Rules
...
included do
  ...
  def print_ability_rules(username)
    ...
    puts "\n"
    puts color "--Ability rules for #{user.first_name.inspect}".ljust(80,'-'), :cyan, :bold
    rules_hash.keys.sort.each do |name|
      hash = rules_hash[name]
      puts "\n"
      puts color "  #{name}", :green
  ...
  def print_cancan_rules(model_class, *args)
    ...
    puts "\n"
    puts color("--#{model_class.name.inspect} ability rules".ljust(80,'-'), :light_cyan)
    args.each do |username|
      user  = get_user(username)
      puts "\n"
      puts color("--cancan rules for #{user.first_name.inspect}:".ljust(40,'-'), :light_green)
      puts "\n"
