module Totem::Test; module Routes; class Route
  ...
  def add_model_to_dictionary(model)
    raise "Cannot add model #{model_class.inspect} to the dictionary.  Dictionary is blank. Model: #{model.inspect}"  if dictionary.blank?
    raise "Cannot add model #{model_class.inspect} to the dictionary.  Model is blank. Model: #{model.inspect}"  if model.blank?
    raise "Model for class #{model_class.inspect} already exists. Model: #{model.inspect}"  if dictionary.has_key?(model.class)
    dictionary[model.class] = model
  end

  def model_to_path(model_for_path=model); model_for_path.class.name.underscore; end
