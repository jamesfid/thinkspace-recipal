    ...
    module Session
      module Api
        ...
        class AuthenticationController < ::Totem::Settings.class.totem.application_controller
          ...
          before_filter :totem_authenticate_user_from_token!, except: [:sign_in, :sign_out]

          private

          def default_to_json; request.format = :json if params[:format].nil?; end
          ...
          rescue_from SessionPasswordStrengthError do |e|
            set_current_user(nil)
            path    = request.original_fullpath
            message = :password_strength_error
            render json: { :path => path, message: message, errors: {user_message: 'Must use a stronger password.'} }, status: 511
          end
          ...
          def validate_password(password)
            raise SessionPasswordStrengthError unless ::Totem::Authentication::Session::Controllers::Password.valid?(password)
          end
