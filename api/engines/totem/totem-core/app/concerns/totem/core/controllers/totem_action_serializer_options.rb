    ...
    module Controllers
      module TotemActionSerializerOptions
        ...
        module ClassMethods
          ...
          # Options:
          #   * Rails standard before_filter options 'only', 'except', 'if', 'unless'
          ...
          #   * before_filter: [string|symbol|true|false] default true for controller; false for options' add: hashes
          #:                   * if true, call serializer options module's 'action' method in before_filter e.g. index, show, ...
          #:                   * if false, must manually call in controller code
          #:                   * if string|symbol (same as true), call the string|symbol method instead of the 'action' method in the before_filter
          ...
          #:                   ---before_filter: '[resource]_[action]_special_options' -> 'users_index_special_options'
          #   * add:           [hash|array|string|symbol]
          #:                   * hash of :name, :module, :module_name and :before_filter
          ...
          def totem_action_serializer_options(*args)
            ...
            before_method = options[:prepend] ? :prepend_before_filter : :before_filter
            self.send(before_method, options.slice(:only, :except, :if, :unless)) do |controller|
              totem_serializer_options.before_filter_process(controller)
            end
          end
          ...
          class TotemControllerSerailizerOptions
            ...
            def initialize(controller_class, options={})
              ...
              set_options_before_filter(options, true)
              add_modules(options)
            end

            # Public method called by the before_filter to call the module methods.
            def before_filter_process(controller)
              serializer_options_methods.each do |method_name, hash|
                before_method = hash[:before_filter]
            ...
            # Allows 'totem_serializer_options' module methods to be called by controller code outside of before_filter.
            ...
            def set_options_before_filter(options, default=false)
              method = options[:before_filter]
              return if method == false
              method = '[action]' if method == true || (method.blank? && default.present?)
              return if method.blank?
              raise_error "before_filter: #{method.inspect} must be a string or symbol."  unless valid_value?(method)
              options[:before_filter] = method.to_s.gsub('[resource]', controller_resource_name)
            end
            ...
            def add_serializer_options_instance_methods(options)
              ...
              set_options_before_filter(options)
              serializer_options_methods[name] = {
                instance:      inst,
                before_filter: options[:before_filter],
              }
            ...
            def print_debug_add(name, mod, options)
              ...
              debug_message "    before filter: #{options[:before_filter].inspect}"
            end
