module Totem; module Core; module Controllers; module ApiRender
  ...
  def controller_render_error(record, options={})
    options[:status] ||= 422
    options[:json]   ||= {:errors => record.errors}
    render options
  end
  ...
  # Record plural key set to empty array and singular key deleted.
  def controller_view_json(record, options={})
    ...
    json.keys.each do |key|
      json.delete(key)  if delete_keys.find {|k| key.to_s.end_with?(k.to_s)}.present?
    end
    json.delete(controller_singular_path) unless options[:keep_singular]
    json[controller_plural_path] = []
    json
  end
