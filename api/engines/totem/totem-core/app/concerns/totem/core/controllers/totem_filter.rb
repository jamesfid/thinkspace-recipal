        ...
        module ClassMethods
          ...
          class TotemControllerFilter
            ...
            class TotemFilter
              ...
              def add_result(key, values, options={})
                values = Array.wrap(values)
                @preserved_order = values if options[:preserve_order]
                @current_results[key] ||= []
              ...
              def generate_scope
                ...
                preserve_order if @preserved_order.present?
                set_scope(@scope)
              end

              def preserve_order
                order_clause = 'CASE id '
                @preserved_order.each_with_index do |id, index|
                  order_clause << "WHEN #{id} THEN #{index} "
                end
                order_clause << "ELSE #{@preserved_order.length} END"
