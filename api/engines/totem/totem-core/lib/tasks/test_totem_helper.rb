...
module TestTotemHelper
  ...
  def totem_get_options(args, options={})
    ...
    list_dirs  = true  # default
    path       = Array.new
    [args.extras].flatten.compact.each do |arg|
      case arg.downcase
      when 'l', 'list', 'list_files', 'list-files'  then list_files = true
      when 'nl', 'no_list', 'no-list'               then list_files = false
      when 'dl', 'dir_list', 'dir-list'             then list_dirs  = true
      when 'ndl', 'no_dir_list', 'no-dir-list'      then list_dirs  = false
      else
        path.push(arg)
      end
    end
  ...
  def totem_setup_rake_test_task(test_files, libs, options)
    ...
    Rake::TestTask.new do |t|
      t.libs       = libs                      # add helpers load path
      t.test_files = test_files                # test files
      t.warning    = ENV['WARNING'] == 'true'  # sets ruby -w
      t.verbose    = false                     # instead use: rake TESTOPTS='--verbose' totem:test:...
    end
