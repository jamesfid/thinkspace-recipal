...
class Test::ActionAuthorize::ControllerBase
  cattr_accessor :before_filter_block
  def self.new_aa(options)
    send :totem_action_authorize!, options
    self.new
  end
  def self.before_filter(options, &blk)
    self.before_filter_block = blk
  ...
  def run_before_filter(controller)
    before_filter_block.call(controller)
  end
end
