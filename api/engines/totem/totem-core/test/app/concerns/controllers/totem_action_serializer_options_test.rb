...
def assert_before_filter(methods, key, value)
  hash = methods[key]
  assert_equal value, hash[:before_filter], 'before filter matches'
end
...
describe 'totem serializer options' do
  ...
  describe 'controller values'  do
    ...
    it 'users with before filter' do
      options = {before_filter: :test_method}
      methods = assert_serializer_options_methods(:users, options)
      assert_before_filter(methods, :users, 'test_method')
    end

    it 'users with before filter replace resource' do
      options = {before_filter: '[resource]_test_method'}
      methods = assert_serializer_options_methods(:users, options)
      assert_before_filter(methods, :users, 'users_test_method')
    end
    ...
    it 'users with module false with other values' do
      options = {name: :test, module_name: :another_one, before_filter: :test_before_filter, module: false}
      methods = get_serializer_options_methods(options)
      assert_equal true, methods.blank?, 'does not add a module'
    end
  ...
  describe 'controller with add key'  do
    ...
    it 'users default and another one default' do
      ...
      assert_before_filter(methods, :users, '[action]')
      assert_before_filter(methods, :another_one, nil)
    end
    ...
    it 'add is array of string and symbol - set as name and module_name' do
      ...
      assert_before_filter(methods, :users, '[action]')
      assert_before_filter(methods, :another_one, nil)
      assert_before_filter(methods, :another_two, nil)
    end
    ...
    it 'users module false and another one before filter as hash' do
      options = {module: false, add: {name: :another_one, before_filter: true}}
      methods = assert_serializer_options_methods([:another_one], options)
      assert_before_filter(methods, :another_one, '[action]')
    end
  ...
  def assert_before_filter_args(action)
    ...
    rc         = controller.tso.before_filter_process(controller)
  ...
  describe 'instance arg arity' do
    ...
    it 'zero' do
      assert_before_filter_args :zero_args
    end

    it 'one' do
      assert_before_filter_args :one_args
    end

    it 'two' do
      assert_before_filter_args :two_args
    end

    it 'three not allowed' do
      e = assert_raises(so_error) {assert_before_filter_args :three_args}
