module Thinkspace::Test; module Controller; module UserLoop
extend ActiveSupport::Concern
included do

  def self.get_mod(mod); "::Thinkspace::Test::Controller::#{mod}".constantize; end
  def self.run_test_it(test_it); [test_it].flatten.compact.map {|ti| self.instance_eval(&ti)}; end

  def self.run_tests_user_loop(config, action, options={})

    mod            = options[:mod] || :TestAction
    test_it        = options[:test_it]
    test_it_before = options[:test_it_before]
    test_it_after  = options[:test_it_after]

    if (readers = config.readers).present?
      readers.each do |username|
        route = config.controller_action_route(action)
        route.set_as_reader
        before_test_it = test_it_before; after_test_it  = test_it_after || test_it
