...
module Thinkspace
  module Importer
    class File < ActiveRecord::Base
      ...
      validates_attachment_content_type :attachment, content_type: %w(text/csv text/x-csv text/plain application/octet-stream application/vnd.ms-excel)
      ...
      def process_csv(csv)
        ...
        get_generated_model_class.transaction do
          csv.each do |row|
            ...
            record = save_csv_generated_record(record)
            records << {record: record, row: row}
          end
        end
        records
      end
