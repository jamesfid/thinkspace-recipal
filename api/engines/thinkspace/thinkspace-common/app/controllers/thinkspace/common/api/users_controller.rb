  ...
  module Common
    module Api
      class UsersController < Totem::Settings.class.thinkspace.authorization_api_controller
        skip_before_filter :verify_authenticity_token, only: [:sign_in, :sign_out, :create]
        ...
        def create
          validate_password(get_password_from_params)
        ...
        def list_keys
          controller_render(@user.thinkspace_common_keys)
        end
        ...
        def get_password_from_params; params_root[:password]; end
        ...
        def add_key_to_user
          key = params[:key]
          return permission_denied('No key was provided.') unless key.present?
          record = Thinkspace::Common::Key.find_by(key: key)
          return permission_denied('No valid key was provided.') unless record.present?
          return permission_denied('You already have added that key.') if @user.thinkspace_common_keys.include?(record)
          record.set_expires_at!
          @user.thinkspace_common_keys << record
        end
