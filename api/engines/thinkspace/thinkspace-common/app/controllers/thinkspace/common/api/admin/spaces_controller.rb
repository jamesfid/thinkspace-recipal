    ...
    module Api
      module Admin
        class SpacesController < ::Totem::Settings.class.thinkspace.authorization_api_controller
          ...
          def import
            ...
            begin
              attachments.each do |attachment|
                file = file_class.new(generated_model: user_class.name, settings: settings, attachment: attachment)
                file.save!
                files << file
              end
          ...
          def import_teams
            settings    = get_import_teams_settings
            attachments = params[:files]
            files       = []
            begin
              attachments.each do |attachment|
              ...
              files = process_roster_files(files)
              @space.delay.import_teams(files, current_user)
              controller_render_json({})
            rescue => e
              permission_denied(get_message_for_import_error(e))
            end
          ...
          def process_roster_files(files)
            ...
            files.each do |file|
              ...
              file_data << {file: file, data: data.read}
            end
            file_data
          end
          ...
          def get_import_settings
            { headers:  
              {
                valid: ['email', 'first_name', 'last_name'], required: ['email'] 
              },
              save: false
            }
          end

          def get_import_teams_settings
            { headers:  
              {
                valid: ['email', 'first_name', 'last_name', 'team'], required: ['email', 'team'] 
              },
              save: false
            }
          end
