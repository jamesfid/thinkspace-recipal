module Thinkspace
  module Common
    class NotificationMailer < ActionMailer::Base
      ...
      def teams_imported(sender, status, team_set, space)
        @to        = sender.email
        @state     = status
        @success   = status.blank?
        @invitable = space
        @team_set  = team_set
        @url       = "#{get_host}/casespace/case_manager/spaces/#{space.id}/team_sets/#{team_set.id}"
        subject    = 'Teams and Roster Imported'
        mail(to: @to, subject: format_subject(subject))
      end
      ...
      def get_host
        ...
        return Rails.application.secrets.smtp['postmark']['domain'] if Rails.env.production?
        ''
      end
