module Thinkspace; module Reporter
class Report < ActiveRecord::Base
  ...
  def self.scope_valid
    joins(:thinkspace_reporter_report_tokens).
    where('thinkspace_reporter_report_tokens.expires_at > ?', Time.now)
  end

  def self.scope_group_by_report_id
    group('thinkspace_reporter_reports.id')
  end
  ...
  # ### Scoped attributes
  def token(scope)
    report_token = thinkspace_reporter_report_tokens.where(thinkspace_common_user: scope.current_user).scope_valid.first
  ...
  def add_file(options)
    file           = Thinkspace::Reporter::File.new(options)
  ...
  def generate_report_token
    Thinkspace::Reporter::ReportToken.create_for_report(self)
  end
  ...
  def send_error_notification(error_code, message='There has been an error when generating your report.')
    ...
    Thinkspace::Reporter::NotificationMailer.report_generation_failed(self, error_code, message).deliver_now
  end
