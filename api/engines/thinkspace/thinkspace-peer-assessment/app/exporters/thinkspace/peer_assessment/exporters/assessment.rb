...
module Thinkspace; module PeerAssessment; module Exporters; class Assessment < Thinkspace::Common::Exporters::Base
  ...
  def initialize(caller, assessment, phase, users)
    ...
    @team_sets   = assessment.thinkspace_peer_assessment_team_sets.where(team_id: team_ids)
  ...
  def process
    ...
    process_qualitative if @assessment.has_qualitative_items? 
  end
  ...
  def get_anonymized_review_json_for_ownerable(ownerable)
    ...
    other_review_sets = @review_sets.scope_by_team_sets(team_set).scope_where_not_ownerable_ids(ownerable).scope_by_valid_for_calculations
    reviews           = @reviews.scope_by_review_sets(other_review_sets).scope_by_reviewable(ownerable).scope_by_valid_for_calculations
