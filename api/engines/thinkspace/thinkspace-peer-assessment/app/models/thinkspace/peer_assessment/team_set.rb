    ...
    class TeamSet < ActiveRecord::Base
      ...
      aasm column: :state do
        ...
        event :mark_as_sent do
          transitions from: [:approved, :sent], to: :sent, after: :mark_as_sent_review_sets
        end
      end
      ...
      def mark_as_sent_review_sets
        thinkspace_peer_assessment_review_sets.scope_approved_or_sent.each { |review_set| review_set.mark_as_sent! if review_set.may_mark_as_sent? }
        thinkspace_peer_assessment_review_sets.scope_not_approved_or_sent.each  { |review_set| review_set.ignore! if review_set.may_ignore? }
      end
