    ...
    module Api
      module Admin
        class AssignmentsController < ::Totem::Settings.class.thinkspace.authorization_api_controller
          ...
          def create
            builder_version = params_root[:builder_version]
            if builder_version.present?
              bundle_type = params_root[:bundle_type]
              case bundle_type.to_sym
              when :casespace
                create_casespace
              when :assessment
                create_assessment
              ...
              @assignment = @assignment_class.new # Allow AASM to property set columns, CanCan avoids this somehow.
              ...
              @assignment.title = "New Case - #{Time.now.to_formatted_s(:db)}"
