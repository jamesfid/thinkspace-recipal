module Thinkspace
  module Casespace
    class Assignment < ActiveRecord::Base
      ...
      # ### Timetable helpers
      def get_or_set_timetable_for_self
        Thinkspace::Common::Timetable.find_or_create_by(ownerable: nil, timeable: self)
      end
      ...
      def is_peer_assessment?
        # Checking on the normal PE case.
        thinkspace_casespace_phases.length == 2 && scope_phases_has_assessment.length > 0
      end
      ...
      def get_user_phase_states(phases, ownerable, user, options={})
        get_phase_states(phases, ownerable, user, options.merge(user_ownerables: true))
      end
