  ...
  module Casespace
    class Phase < ActiveRecord::Base
      ...
      def get_ownerables(ownerable, user, options={})
        ...
        ownerables.push(ownerable) if can_update && self.team_ownerable?
        ownerables
      end

      def get_base_ownerables(ownerable, user, options={})
        user_ownerables = options[:user_ownerables] || false
        case
        when self.team_ownerable?        then get_teams(ownerable)
        when ownerable.is_a?(user_class) then Array.wrap(ownerable)
        when ownerable.is_a?(team_class) then user_ownerables ? Array.wrap(user) : ownerable.thinkspace_common_users
        else
      ...
      def unlock_for_ownerable(ownerable, tt, validate=true)
        ...
        time         = Time.now
        phase_states = thinkspace_casespace_phase_states.scope_by_ownerables(ownerable).scope_locked
