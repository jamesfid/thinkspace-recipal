  ...
  module ModelAdapters
    class ActiveRecordAdapter < AbstractAdapter
      ...
      def database_records
        if override_scope
          ...
          if mergeable_conditions
            if joins.present?
              # @model_class.where(conditions).includes(joins)  # original
              # sixthedge change to always 'eager_load' association.
              @model_class.where(conditions).includes(joins).eager_load(joins) # changed by sixthedge
            else
      ...
      # end
       ...
       # Replaced by sixthedge from cancancan (10/23/2015).
      def sanitize_sql(conditions)
